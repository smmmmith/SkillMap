...
export const calculateSkillProgress = (skill) => {
  const totalSubSkills = skill.levels.reduce((total, level) => total + level.subSkills.length, 0);
  const completedSubSkills = skill.levels.reduce((total, level) => 
    total + level.subSkills.filter(subSkill => subSkill.completed).length, 0);
  return Math.round((completedSubSkills / totalSubSkills) * 100);
};